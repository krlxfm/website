...
@section('head')
    ...
    <div class="row mt-4">
        <div class="col-md">
            ...
            <ol>
                <li><strong>Review the position(s) you're interested in.</strong> All positions have their responsibilities outlined <a href="{{ route('board.positions') }}">here</a>, though each member makes the position their own. You are strongly encouraged to <a href="{{ route('board.positions') }}">read through the list of positions and their responsibilities</a>, then <a href="{{ route('board.meet') }}">contact the board member(s) who currently hold that position</a> to ask what their experience has been like. Note that <strong>we don't have at-large seats,</strong> meaning you must apply directly to at least one position, but there's no hard limit on how many you can apply for.</li>
